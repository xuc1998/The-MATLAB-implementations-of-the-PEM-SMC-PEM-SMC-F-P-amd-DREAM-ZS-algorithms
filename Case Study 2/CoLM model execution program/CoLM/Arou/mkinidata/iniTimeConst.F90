...
! 18 bare soil
  ...
  integer, parameter :: numIGBP = 17
  real(r8), dimension(numIGBP) :: &!
  z0m_igbp        , &!roughness
  displa_igbp     , &!zero-plane-distance
  sqrtdi_igbp     , &!inverse sqrt of leaf dimension [m**-0.5]
        
  chil_igbp       , &!leaf angle distribution factor
  ref_s_igbp      , &!leaf reflectance
  ref_sd_igbp     , &!leaf reflectance
  ref_l_igbp      , &!leaf reflectance
  ref_ld_igbp     , &!leaf reflectance
  tran_s_igbp     , &!leaf transmittance
  tran_sd_igbp    , &!leaf transmittance
  tran_l_igbp     , &!leaf transmittance
  tran_ld_igbp    , &!leaf transmittance
 
  vmax0_igbp      , &!maximum carboxylation rate at 25 C at canopy top
  effcon_igbp     , &!quantum efficiency
  gradm_igbp      , &!conductance-photosynthesis slope parameter
  binter_igbp     , &!conductance-photosynthesis intercept
  respcp_igbp     , &!respiration fraction
  shti_igbp       , &!slope of high temperature inhibition function (s1)
  slti_igbp       , &!slope of low temperature inhibition function (s3)
  trda_igbp       , &!temperature coefficient in gs-a model (s5)
  trdm_igbp       , &!temperature coefficient in gs-a model (s6)
  trop_igbp       , &!temperature coefficient in gs-a model (273.16+25)
  hhti_igbp       , &!1/2 point of high temperature inhibition function (s2)
  hlti_igbp       , &!1/2 point of low temperature inhibition function (s4)
  extkn_igbp      , &!coefficient of leaf nitrogen allocation
 
  d50_igbp        , &!depth at 50% roots
  d95_igbp        , &!depth at 95% roots
  beta_igbp          !coefficient of root profile
...
!10 Water, Wetlands, Ice/Snow
  ...
  integer, parameter :: numSiB2 = 10
  real(r8), dimension(numSiB2) :: &!
  z0m_sib2        , &!roughness
  displa_sib2     , &!zero-plane-distance
  sqrtdi_sib2     , &!inverse sqrt of leaf dimension [m**-0.5]
              
  chil_sib2       , &!leaf angle distribution factor
  ref_s_sib2      , &!leaf reflectance
  ref_sd_sib2     , &!leaf reflectance
  ref_l_sib2      , &!leaf reflectance
  ref_ld_sib2     , &!leaf reflectance
  tran_s_sib2     , &!leaf transmittance
  tran_sd_sib2    , &!leaf transmittance
  tran_l_sib2     , &!leaf transmittance
  tran_ld_sib2    , &!leaf transmittance
 
  vmax0_sib2      , &!maximum carboxylation rate at 25 C at canopy top
  effcon_sib2     , &!quantum efficiency
  gradm_sib2      , &!conductance-photosynthesis slope parameter
  binter_sib2     , &!conductance-photosynthesis intercept
  respcp_sib2     , &!respiration fraction
  shti_sib2       , &!slope of high temperature inhibition function (s1)
  slti_sib2       , &!slope of low temperature inhibition function (s3)
  trda_sib2       , &!temperature coefficient in gs-a model (s5)
  trdm_sib2       , &!temperature coefficient in gs-a model (s6)
  trop_sib2       , &!temperature coefficient in gs-a model (273.16+25)
  hhti_sib2       , &!1/2 point of high temperature inhibition function (s2)
  hlti_sib2       , &!1/2 point of low temperature inhibition function (s4)
  extkn_sib2      , &!coefficient of leaf nitrogen allocation
 
  d50_sib2        , &!depth at 50% roots
  d95_sib2        , &!depth at 95% roots
  beta_sib2          !coefficient of root profile
...
!     print*,'iniTimeConst.F90-test-tran(2,2)-35',tran(2,2)    
 ...
 stop 'do nothing on IGBP type'

#elif (defined SiB2)      

 stop 'do nothing on SiB2 type'
