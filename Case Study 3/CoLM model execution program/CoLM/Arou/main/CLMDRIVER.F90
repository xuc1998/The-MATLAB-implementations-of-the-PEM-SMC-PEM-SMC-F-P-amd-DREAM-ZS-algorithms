...
#if(!defined EcoDynamics)
  real(r8), INTENT(in) :: flai (numpatch,4)        ! read-in time-varying vegetation parameters
#endif
...
#if (!defined EcoDynamics)
  real(r8) lai_r(numpatch)     ! read-in leaf area index
  real(r8) sai_r(numpatch)     ! read-in stem+dead leaf area index
  real(r8) green_r(numpatch)   ! read-in leaf greenness
  real(r8) fveg_r(numpatch)    ! read-in vegetation fraction cover
#endif
...
#if(!defined EcoDynamics)
      lai_r(i), sai_r(i), green_r(i), fveg_r(i), &
#endif
