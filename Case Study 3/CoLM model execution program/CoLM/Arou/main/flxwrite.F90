...
subroutine Myoutputwrite_vector(lsao,mstep,output_data)
!---------------------------------------------------------------------------
!description: output my interested variables(vector)
!original author:Cong Xu
    ...
    integer i
    real(r8):: output_data(mstep)
    do i=1,mstep
       write(lsao,*) output_data(i)
    end do
    close(lsao)
end subroutine Myoutputwrite_vector

subroutine Myoutputwrite_matrix(lsao,mstep,ncol,output_data)
!------------------------------------------------------------------------------
! description: output my interested variables (matrix)
! original author: Cong Xu
    use precision
    implicit none
    integer, INTENT(in):: lsao
    ...
    real(r8),dimension(mstep,ncol),INTENT(in):: output_data
    integer :: i,j
    do i=1, mstep
       do j=1,ncol
           write(lsao,*) output_data(i,j)
       end do
    end do
    close(lsao)
end subroutine Myoutputwrite_matrix
