...
#if(defined RDGRID)
      lugrid = 10
      OPEN(lugrid,file=fgridname,form='formatted',status='old')
      CALL rdgrid (lugrid,edgen,edgee,edges,edgew,lon_points,lat_points,&
...
!            print*,'mksrfdata.F90-test-veg_numtype',numtype(i,j,:)
          ...
          ! meteorological observation (grass_id+1=8 for USGS classification)
            do L = 1, nlandcateg
               if(fpatch2d(i,j,L).lt.1./100. .and. L.ne.9)then
            ...
            if(fpatch2d(i,j,1).lt.1.0e-6)then
               if(fpatch2d(i,j,9).lt.1./100.)then 
                  Loca = maxloc(fpatch2d(i,j,:))
                  LL=Loca(1) 
                  fpatch2d(i,j,LL) = fpatch2d(i,j,LL) + fpatch2d(i,j,9) - 1./100.
                  fpatch2d(i,j,9) = 1./100.
               endif
            endif
...
SUBROUTINE MKSRFDATA (fgridname, fdemname, fmaskname, &
                      flandname, fsolaname, fsolbname, fsurdat, &
                      lon_points, lat_points, edgen, edgee, edges, edgew)
      ...
      do j = 1, lat_points
         do i = 1, lon_points
          ...
          ! meteorological observation (grass_id+1=8 for USGS classification)
            ...
            enddo
            print*,'fpatch2d(i,j,1):',fpatch2d(1,1,1)
            if(fpatch2d(i,j,1).lt.1.0e-6)then
