...
for stage=2:S
    ...
    w=w.*exp(cal_weight);
    w=w./(sum(w));
    Neff(stage,1)=1./sum(power(w,2)); % effective sample size
   % Resampling
   ...
   for i=1:Np
        ...
        alp_media=betas_new*target(parameter_median);    
         % M-H accept
   ...
   % Crossover Operater
    ...
    for k=1:Np/2        
        ...
        alp_old=betas_new*(target(parameter_new(old_ind(1),:))+target(parameter_new(old_ind(2),:)));               
        alp_media=betas_new*(target(parameter_median(2*k-1,:))+target(parameter_median(2*k,:))); 
        % M-H accept
        ratio=min([1,exp(alp_media-alp_old)]);
        u=rand;
        if u<ratio            
            ...
            accept_c(stage,1)=accept_c(stage,1)+2;
        else
    ...
    for k=1:Np    
        ...
        alp_old=betas_new*target(parameter_new(k,:));
...
while 1
    ...
    for j=1:d
        parameter_new(1,j)=parameter_old(k,j)+gamma*(parameter_old(R1,j)-parameter_old(R2,j))+b*randn;
    end    
